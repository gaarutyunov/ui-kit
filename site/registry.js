...
--ga-metric-label-size    /* label scale (default --ga-fs-xs)             */</code></pre>
  ...
  slider: {
    title: "Slider",
    tag: "ga-slider",
    lead: "A range slider with an optional label and live value readout. Form-associated, because it is a native <input type=range>. Reach for it whenever you want one value. Name the two ends with label-start / label-end for the \"this ↔ that\" preference slider where the number itself means nothing. For a segmented media timeline — coloured intervals, per-segment activation, an independent playhead — reach for ga-scrubber instead: a range input has no per-region children and exactly one movable mark, so it cannot render that at any price.",
    examples: [
